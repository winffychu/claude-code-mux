...
- router 命中的 target model 未在 `[[models]]` 配置 + `provider_registry` 也找不到
  → server/mod.rs `fn handle_openai_chat_completions` 的 **`❌ No model mapping or provider found`** HTTP 502
  返回，**不退 default**（fail-fast；路由决策已下，layer fallback 不会跳回路由层）。
- 命中 model 的 mapping 在 `[[models]]` 配置了但上游 provider 调用失败
  → server/mod.rs `fn handle_openai_chat_completions`：按 priority 顺序 **1:N mapping retry** (`[idx+1/N]`
...
- 注意：`router.rules` 含一条 `prefix = "broken-"` → `model = "nonexistent-model"`，
  `nonexistent-model` **不**在 `[[models]]` 配置（用来触发 `fn handle_openai_chat_completions` 的 fail-fast）。
...
- **注意**：单 mapping model（如 `default-model` 本身配置只 1 个 nvidia provider）失败
  ...
  整请求 502 — 见 server `fn handle_openai_chat_completions` 的 `❌ All 1 provider mappings failed`. 同样 fail-fast 不跳回
  路由层的 next branch.
- 本节未覆盖的边缘：上游返回非 200 + 仍 parsed JSON 但内容异常（如部分截断
  /上游反 401 但附 HTML body）。ccm 对所有 transport-level 错都进 `trying next fallback`
  分支（见 `fn handle_openai_chat_completions` 内的 `trying next fallback`），不依赖响应体内容语义。
...
（与 server/mod.rs `fn handle_openai_chat_completions` 的 `❌ All N provider mappings failed` 完全一致），**不退 default**。cf=false 和 cf=true 行为

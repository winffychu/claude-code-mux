...
> 状态：P4a 降级版已实施（2026-07-10），P4a-SSE / P4b 未实施
...
- **⚠️ 部分实现** (`openai_compat.rs` L217-266): 响应方向 stop_reason 映射、**仅 text content** 转换；**tool_use content blocks 被静默丢弃**（L224-227 只 filter_map text）；**finish_reason 缺 `tool_use → tool_calls` 映射**（L236-243 只有 end_turn/max_tokens/stop_sequence）
- **❌ 缺失**: `tools: None` (L212) — tool **定义**未转换（OpenAI function.format → Anthropic Tool）
- **❌ 缺失**: `OpenAIMessage` 无 `tool_calls` 字段 — assistant 历史中的 tool_calls 被跳过（L193 `_ => skip`）
- **❌ 缺失**: `OpenAIMessage` 无 `tool` role 处理 — OpenAI `tool` role 消息（tool_result）被跳过
- **❌ 缺失**: `OpenAIResponseMessage` 无 `tool_calls` 字段 — 响应中 tool_use 无法返回给 OpenAI client
- **❌ 缺失**: streaming SSE 转换 — `openai_compat.rs` 当前不处理 SSE streaming——stream 字段被透传至 AnthropicRequest 但 handler 不做 SSE 事件转换，streaming 请求会因 Anthropic 端返回 SSE 而 OpenAI client 无法解析
- **❌ 缺失**: Gemini 入站端点 (`server/mod.rs` 无 `/v1beta/models/:model:generateContent` 路由)

> **四轮审计修正（2026-07-10）**：原文档仅识别 1 个缺口（L212 tools: None），实际代码审查发现 **5 个缺口**：
> 1. L212 tool 定义转换（原文档已列）
> 2. `OpenAIMessage` 结构缺 `tool_calls` 字段（assistant 历史中的工具调用丢失）
> 3. `OpenAIMessage` 缺 `tool` role 处理（tool_result 消息丢失）
> 4. `OpenAIResponseMessage` 缺 `tool_calls` 字段（响应中工具调用无法返回）
> 5. `finish_reason` 缺 `tool_use → tool_calls` 映射
>
> 依据：`grep -n 'tool_calls\|tool' src/server/openai_compat.rs` 确认 OpenAIMessage (L28-35) 和 OpenAIResponseMessage (L81-84) 均无 tool_calls 字段；transform_anthropic_to_openai (L224-227) 只提取 text content。
...
| openai_chat_completions | anthropic_messages | 逆向（入站） | ⚠️ 部分 | `openai_compat.rs` L93-266 | 请求/响应基本已实现；**缺 5 项工具相关转换**（见上方四轮审计修正）+ 缺 SSE streaming |
| openai_chat_completions | openai_responses | 横向 | ❌ | — | |
| openai_chat_completions | gemini_generate_content | 横向 | ❌ | — | |
| openai_responses | anthropic_messages | 逆向（入站） | ❌ | — | |
| gemini_generate_content | anthropic_messages | 逆向（入站） | ❌ | — | server 无 Gemini 入站端点 |

> **审计修正**：原文档将 `openai_chat_completions → anthropic_messages` 标为 ❌，实际 `openai_compat.rs` L93-214 已实现请求转换（system 提取、user/assistant 转换、image 转换），L217-266 已实现响应转换（stop_reason 映射、content 转换）。正确状态为 ⚠️ 部分实现。

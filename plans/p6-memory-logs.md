...
> 状态：**已实施**（commit `6b877af`，2026-07-16；`RUSTFLAGS=-D warnings` 测试 255→271 passed）；本 plan 文档 2026-07-18 R5 文档审计时 status 由 "待实施" 同步标 "已实施"
...
- **fallback 链每尝试一个 provider mapping 就记一条 `req` trace 条目**：
  `trace_request` 在 `src/server/mod.rs` 的 fallback for 循环内调用（L671
  OpenAI 端点、L947 Anthropic 端点），与 stdout `info!` 的 `[n/N]` 重试
